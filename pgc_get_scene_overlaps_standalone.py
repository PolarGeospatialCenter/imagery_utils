...
import os, sys, string, shutil, glob, re, socket, fnmatch, math, argparse
...
def main():
	...
	description="Build of text file of stereopair scene overlaps"
	)
    ...
    #### Look for images (case insentitive), parent dir is pairname
    scenes = {}
    for root,dirs,files in os.walk(src):
        for f in files:
            if f.endswith((".NTF",".ntf",".TIF",".tif")) and not 'browse' in f.lower():
            # "P1BS" in f and
                #print f
                
                ### check for duplicate scnee IDs (both a nitf and a tiff file of same base name) and prefer ntfs
    ...
    for sceneid, scenepath in scenes.iteritems():
        ...
        if stereo_image.isvalid is True:
            if stereo_image.source.bands == 1:  #### return only Pan images
                ...
                print "Stereo image rejected due to band constraint: %s has %d bands" %(stereo_image.path,stereo_image.source.bands)
        else:
    ...
    if len(stereo_images) > 0:
        ...
        for img in stereo_images:
            if img.pairname in pn_img_dict:
                imglist = pn_img_dict[img.pairname]
                imglist.append(img)
                pn_img_dict[img.pairname] = imglist
            else:
                pn_img_dict[img.pairname] = [img]

        #print pn_img_dict
        stereopairs = []
        for pairname, pair_images in pn_img_dict.iteritems():
            stereopair = StereoPair(pairname, pair_images, "scene")
    ...
    for stereopair in stereopairs:
        ...
        overlaps = sorted(stereopair.overlaps, key=lambda overlap: overlap.name)
...
class RasterInfo(object):
    ...
    def __init__(self, name, geom, srs, bands, elev=None, pixelcount=None, linecount=None, xres=None, yres=None, coords=None):
...
class StereoImage(object):
    ...
    def __init__(self, image_path, pairname, tgt_srs, rasterinfo=None):
        ...
        if self.source:
            ...
            else:  ### if files are not on an available filesystem, xml path is assumed to be the same as the file path with a lower case extension
                self.metapath = os.path.join(self.dir,self.basename+".xml")
...
class StereoPair(object):
    ...
    def __init__(self, pairname, stereo_images, sp_type):
        ...
        if len(rasters_band_counts) != 1:
            print "Images in this stereopair have varying numbers of bands.  This will break things.  Try running the script with the band_constraint option.\n\t{pairname} - {bandset}".format(pairname=self.pairname, bandset=str(rasters_band_counts))
            self.isvalid = False
    ...
    def _buildOverlaps_mosaic(self):
        ...
        overlap = Overlap(overlap_images, overlap_geom, overlap_percent,  overlap_geom.Area(), self.pairname, self.sp_type, self.pairname)
                ...
                for img2 in self.stereo_images:
                    if img2.catid != img1.catid:
                        ...
                        if img1.target.geom.Intersects(img2.target.geom):
                            ...
                            # Overlap name: WV01_20120603_102001001B4BFB00_102001001BE7F800_R1C1-052903570060_01_P001_052903564030_01_P001
                            ...
                            if overlap_percent >= 0.10:
                                overlap = Overlap(overlap_images, overlap_geom, overlap_percent, overlap_geom.Area(), overlap_name, self.sp_type, self.pairname)
